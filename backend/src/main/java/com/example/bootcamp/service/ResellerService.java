...
public class ResellerService {
                ...
                return ordersPage.map(order -> {
                        ...
                        for (OrderItemsEntity item : order.getOrderItems()) {
                                ...
                                orderTotalProfit = orderTotalProfit
                                                .add(item.getSellingPrice().subtract(item.getCostPrice())
                                                                .multiply(qty));

                                if (productName.length() > 0)
                                        productName.append(" , ");
                                productName.append(item.getProductName()).append(" (").append(item.getQuantity())
                                                .append(")");
                        }
                ...
                for (int i = 6; i >= 0; i--) {
                        ...
                        for (OrdersEntity o : allOrders) {
                                if (o.getCreatedAt() != null) {
                                        java.time.LocalDate orderDate = o.getCreatedAt()
                                                        .atZone(java.time.ZoneId.systemDefault())
                                                        .toLocalDate();
                                        if (orderDate.equals(targetDate)) {
                                                dailyAmount = dailyAmount
                                                                .add(o.getTotalAmount() != null ? o.getTotalAmount()
                                                                                : BigDecimal.ZERO);
                                                dailyProfit = dailyProfit
                                                                .add(o.getResellerProfit() != null
                                                                                ? o.getResellerProfit()
                                                                                : BigDecimal.ZERO);
                                        }
                                }
        ...
        public ResellerWalletResponse getWalletDetails(String email, Pageable pageable) {
                ...
                if (shop != null) {
                        ...
                        for (OrdersEntity o : ordersPage.getContent()) {
                                if (o.getStatus() == OrdersEntity.Status.shipped
                                                || o.getStatus() == OrdersEntity.Status.completed) {
        ...
        @Transactional
        public long getUnreadOrderCount(String email) {
                UsersEntity user = userRepository.findByEmail(email)
                                .orElseThrow(() -> new RuntimeException("ไม่พบผู้ใช้งาน"));
                ShopsEntity shop = shopRepository.findByUserId(user.getId())
                                .orElseThrow(() -> new RuntimeException("ไม่พบร้านค้าของคุณ"));
        ...
        @Transactional
        public void markOrdersAsRead(String email) {
                UsersEntity user = userRepository.findByEmail(email)
                                .orElseThrow(() -> new RuntimeException("ไม่พบผู้ใช้งาน"));
                ShopsEntity shop = shopRepository.findByUserId(user.getId())
                                .orElseThrow(() -> new RuntimeException("ไม่พบร้านค้าของคุณ"));
